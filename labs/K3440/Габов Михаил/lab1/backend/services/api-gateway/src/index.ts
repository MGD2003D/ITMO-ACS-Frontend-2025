...
app.use('/auth-api-docs', proxy(services.auth, {
    proxyReqPathResolver: (req: Request) => {
        const url = req.originalUrl.replace('/auth-api-docs', '/api-docs');
        console.log(`[Proxy] /auth-api-docs -> ${url}`);
        return url;
    }
}));
app.use('/recipes-api-docs', proxy(services.recipes, {
    proxyReqPathResolver: (req: Request) => {
        const url = req.originalUrl.replace('/recipes-api-docs', '/api-docs');
        console.log(`[Proxy] /recipes-api-docs -> ${url}`);
        return url;
    }
}));
app.use('/interactions-api-docs', proxy(services.interactions, {
    proxyReqPathResolver: (req: Request) => {
        const url = req.originalUrl.replace('/interactions-api-docs', '/api-docs');
        console.log(`[Proxy] /interactions-api-docs -> ${url}`);
        return url;
    }
}));
